...
fun AppNavHost(
    ...
    androidx.navigation.compose.NavHost(
        ...
        startDestination = start.destinationId(),
        modifier = modifier,
        builder = builder
    )
}
...
fun NavGraphBuilder.composable(
    ...
    content: @Composable (Map<AppNavArg, Any?>) -> Unit
) = composable(
    route = dest.destinationId(),
    arguments = dest.namedNavArguments(),
    enterTransition = enterTransition,
    exitTransition = exitTransition,
    popEnterTransition = popEnterTransition,
    popExitTransition = popExitTransition,
    content = { navBackStackEntry ->
        val argMap = mutableMapOf<AppNavArg, Any?>()
        dest.arguments().forEach { arg ->
            argMap[arg] = arg.value(navBackStackEntry.arguments)
        }
        content(argMap.toMap())
    }
)